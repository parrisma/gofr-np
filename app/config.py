...
class Config:
    ...
    def get_data_dir(cls) -> Path:
        ...
        Returns:
            Path to data directory (configurable via GOFRNP_DATA_DIR environment variable)

        Default locations:
            - Production: /home/{user}/devroot/gofr-np/data
            - Testing: Temporary directory set by tests
            - Docker: Can be overridden via GOFRNP_DATA_DIR env var
        """
        if cls._test_mode and cls._test_data_dir:
            return cls._test_data_dir

        # Check environment variable for data directory
        env_data_dir = os.environ.get("GOFRNP_DATA_DIR")
        if env_data_dir:
            return Path(env_data_dir)

        # Default to project data directory
        # This assumes we're running from /home/{user}/devroot/gofr-np
    ...
    def get_sessions_dir(cls) -> Path:
        """Get the directory for document session storage."""
